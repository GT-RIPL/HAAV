...
class TxtCtxField(RawField):
    ...
    def __load(self, ctx, k):
        ctx_whole_f = ctx[f"{k}/whole/features"][:8*self.k]
        ctx_whole_p = np.zeros((len(ctx_whole_f), ), dtype=np.int64)
        ctx_whole_r = np.arange(8*self.k)

        ctx_five_f = ctx[f"{k}/five/features"][:, :2*self.k]
        ctx_five_p = np.tile(np.arange(5)[:, None], (1, 2*self.k))
        ctx_five_r = np.tile(np.arange(2*self.k)[None, :], (5, 1))
        ctx_five_f = ctx_five_f.reshape((5*2*self.k, -1))
        ctx_five_p = ctx_five_p.reshape((5*2*self.k, ))
        ctx_five_r = ctx_five_r.reshape((5*2*self.k, ))

        ctx_nine_f = ctx[f"{k}/nine/features"][:, :self.k]
        ctx_nine_p = np.tile(np.arange(9)[:, None], (1, self.k))
        ctx_nine_r = np.tile(np.arange(self.k)[None, :], (9, 1))
        ctx_nine_f = ctx_nine_f.reshape((9*self.k, -1))
        ctx_nine_p = ctx_nine_p.reshape((9*self.k, ))
        ctx_nine_r = ctx_nine_r.reshape((9*self.k, ))

        return {
            "whole": {"features": ctx_whole_f, "positions": ctx_whole_p, "ranks": ctx_whole_r},
            "five": {"features": ctx_five_f, "positions": ctx_five_p, "ranks": ctx_five_r},
            "nine": {"features": ctx_nine_f, "positions": ctx_nine_p, "ranks": ctx_nine_r}
        }
    ...
    def preprocess(self, x):
        if self.preload:
            ...
            ctx_whole_p = self.ctx[str(x)]["whole"]["positions"]
            ctx_whole_r = self.ctx[str(x)]["whole"]["ranks"]
            ctx_five_f = self.ctx[str(x)]["five"]["features"]
            ctx_five_p = self.ctx[str(x)]["five"]["positions"]
            ctx_five_r = self.ctx[str(x)]["five"]["ranks"]
            ctx_nine_f = self.ctx[str(x)]["nine"]["features"]
            ctx_nine_p = self.ctx[str(x)]["nine"]["positions"]
            ctx_nine_r = self.ctx[str(x)]["nine"]["ranks"]
        else:
            ctx = h5py.File(self.ctx, "r")
            data = self.__load(ctx, x)
            ctx_whole_f = data["whole"]["features"]
            ctx_whole_p = data["whole"]["positions"]
            ctx_whole_r = data["whole"]["ranks"]
            ctx_five_f = data["five"]["features"]
            ctx_five_p = data["five"]["positions"]
            ctx_five_r = data["five"]["ranks"]
            ctx_nine_f = data["nine"]["features"]
            ctx_nine_p = data["nine"]["positions"]
            ctx_nine_r = data["nine"]["ranks"]
            ctx.close()

        ctx_whole_f = torch.FloatTensor(ctx_whole_f)
        ctx_whole_p = torch.LongTensor(ctx_whole_p)
        ctx_whole_r = torch.LongTensor(ctx_whole_r)
        ctx_five_f = torch.FloatTensor(ctx_five_f)
        ctx_five_p = torch.LongTensor(ctx_five_p)
        ctx_five_r = torch.LongTensor(ctx_five_r)
        ctx_nine_f = torch.FloatTensor(ctx_nine_f)
        ctx_nine_p = torch.LongTensor(ctx_nine_p)
        ctx_nine_r = torch.LongTensor(ctx_nine_r)

        return ctx_whole_f, ctx_whole_p, ctx_whole_r, ctx_five_f, ctx_five_p, ctx_five_r, ctx_nine_f, ctx_nine_p, ctx_nine_r
            
    def process(self, batch, *args, **kwargs):
        ctx_whole_f, ctx_whole_p, ctx_whole_r, ctx_five_f, ctx_five_p, ctx_five_r, ctx_nine_f, ctx_nine_p, ctx_nine_r = list(zip(*batch))
        
        ctx_whole_f = torch.stack(ctx_whole_f)
        ctx_whole_p = torch.stack(ctx_whole_p)
        ctx_whole_r = torch.stack(ctx_whole_r)

        ctx_five_f = torch.stack(ctx_five_f)
        ctx_five_p = torch.stack(ctx_five_p)
        ctx_five_r = torch.stack(ctx_five_r)

        ctx_nine_f = torch.stack(ctx_nine_f)
        ctx_nine_p = torch.stack(ctx_nine_p)
        ctx_nine_r = torch.stack(ctx_nine_r)

        return {
            "whole": {"embed": ctx_whole_f, "pos": ctx_whole_p, "rank": ctx_whole_r},
            "five": {"embed": ctx_five_f, "pos": ctx_five_p, "rank": ctx_five_r},
            "nine": {"embed": ctx_nine_f, "pos": ctx_nine_p, "rank": ctx_nine_r},
        }
...
class VisCtxField(RawField):
    def __init__(self, ctx_file, preload=False, preprocessing=None, postprocessing=None):
        ...
        f = h5py.File(ctx_file, "r")
        self.fdim_pooled = f.attrs["fdim_pooled"]
        self.fdim_grid = f.attrs["fdim_grid"]
        f.close()
    ...
    def __load(self, ctx, k):
        return {
            "grid_features": ctx[f"{k}/grid_features"][:],
            "pooled_features": ctx[f"{k}/pooled_features"][:]
        }
    ...
    def preprocess(self, image_id):
        if self.preload:
            grid_features = self.ctx[str(image_id)]["grid_features"]
            pooled_features = self.ctx[str(image_id)]["pooled_features"]
        else:
            grid_features = self.ctx[f"{image_id}/grid_features"][:]
            pooled_features = self.ctx[f"{image_id}/pooled_features"][:]
        
        grid_features = torch.FloatTensor(grid_features)
        pooled_features = torch.FloatTensor(pooled_features)

        return grid_features, pooled_features

    def process(self, batch, *args, **kwargs):
        grid_features, pooled_features = list(zip(*batch))
        grid_features = torch.stack(grid_features)
        pooled_features = torch.stack(pooled_features)

        return {"grid": grid_features, "pooled": pooled_features}
