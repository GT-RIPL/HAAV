...
class COCO(PairedDataset):
    def __init__(
        self, fields, ann_root, id_root=None, use_restval=True, cut_validation=False, train_ratio=1.0
    ):
        roots = {}
        ...
        with nostdout():
            self.train_examples, self.val_examples, self.test_examples = self.get_samples(roots, ids, train_ratio)
    ...
    def get_samples(self, roots, ids_dataset=None, train_ratio=1.0):
        train_samples = []
        val_samples = []
        test_samples = []
        ...
        k = round(len(train_samples) * train_ratio)
        train_samples = list(random.sample(train_samples, k))
